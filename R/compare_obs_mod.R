...
#' @param n.col The number of columns used in the facetted plot.
...
compare_obs_mod <- function(data,
  ...
  if (robust) {
    if (identical(group, NA)) {
      ...
      modelLM <- regr_data %>%
        tidyr::nest(data = -rlang::sym(group)) %>%
        dplyr::mutate(fit = purrr::map(data, ~ MASS::rlm(fm, data = ., maxit = maxit)))
    }
  } else {
    if (identical(group, NA)) {
      ...
      modelLM <- regr_data %>%
        tidyr::nest(data = -rlang::sym(group)) %>%
        dplyr::mutate(fit = purrr::map(data, ~ stats::lm(fm, data = .)))
    }
  }
  ...
  if ("lm" %in% class(modelLM)) {
    ...
    for (lm.index in seq(1, nrow(modelLM), 1)) {
      ...
      if (nrow(regression.out) > 0) {
        regression.out <- dplyr::bind_rows(
          regression.out,
          regression.out.tmp
        )
      } else {
    ...
    scatter <- ggplot2::ggplot(
      regr_data,
      ggplot2::aes(x = x, y = y, group = !!rlang::sym(group))
    )
  }
  ...
  if (point.color.in.aes) {
    scatter <- scatter +
      ggplot2::geom_point(
        data = regr_data,
        ggplot2::aes(colour = !!rlang::sym(point.color)),
        shape = point.shape,
        alpha = point.alpha,
        size = point.size
      )
  ...
  max_axis <- max(c(
    ceiling(ggplot2::ggplot_build(scatter)$layout$panel_scales_x[[1]]$range$range / 10) * 10,
    ceiling(ggplot2::ggplot_build(scatter)$layout$panel_scales_y[[1]]$range$range / 10) * 10
  ))
  ...
  if (is.na(group)) {
    ...
    num.group <- length(unique(data[[group]]))
    # to make sure that the lines work, we need to set the scales already
    scatter <- scatter +
      ggplot2::facet_wrap(dplyr::vars(!!rlang::sym(group)), scales = "free", ncol = n.col)
  }

  abline_coord <- list(
    "0.5" = list(
      slope = rep(0.5, num.group),
      intercept = rep(0, num.group),
      label = rep(geomtextpath$labels[["0.5"]], num.group),
      hjust = rep(geomtextpath$hjust[["0.5"]]), num.group
    ),
    "1.0" = list(
      slope = rep(1.0, num.group),
      intercept = rep(0, num.group),
      label = rep(geomtextpath$labels[["1.0"]], num.group),
      hjust = rep(geomtextpath$hjust[["1.0"]]), num.group
    ),
    "2.0" = list(
      slope = rep(2.0, num.group),
      intercept = rep(0, num.group),
      label = rep(geomtextpath$labels[["2.0"]], num.group),
      hjust = rep(geomtextpath$hjust[["2.0"]], num.group)
    )
  )
    ...
    if (requireNamespace("geomtextpath", quietly = TRUE)) {
      scatter <- scatter +
        geomtextpath::geom_labelabline(
          label = abline_coord[["0.5"]]$label,
          slope = abline_coord[["0.5"]]$slope,
          intercept = abline_coord[["0.5"]]$intercept,
          hjust = abline_coord[["0.5"]]$hjust,
          boxcolour = NA,
          boxlinewidth = 0,
          fill = NA,
          gap = TRUE,
        ...
        geomtextpath::geom_labelabline(
          label = abline_coord[["1.0"]]$label,
          slope = abline_coord[["1.0"]]$slope,
          intercept = abline_coord[["1.0"]]$intercept,
          hjust = abline_coord[["1.0"]]$hjust,
          boxcolour = NA,
          boxlinewidth = 0,
          fill = NA,
          gap = TRUE,
        ...
        geomtextpath::geom_labelabline(
          label = abline_coord[["2.0"]]$label,
          slope = abline_coord[["2.0"]]$slope,
          intercept = abline_coord[["2.0"]]$intercept,
          hjust = abline_coord[["2.0"]]$hjust,
          boxcolour = NA,
          boxlinewidth = 0,
          fill = NA,
          gap = TRUE,
    ...
    if (robust) {
      scatter <- scatter +
        ggpmisc::stat_poly_eq(
          mapping = ggpmisc::use_label("eq"),
          formula = fm,
          method = MASS::rlm,
          method.args = list("maxit" = maxit),
          label.x = rep(0.05, num.group),
          label.y = rep(0.90, num.group)
        )
    } else {
      scatter <- scatter +
  ...
  if (asp.ratio.1) {
    ...
    if (!identical(group, NA)) {
      scatter <- scatter +
        ggplot2::facet_wrap(dplyr::vars(!!rlang::sym(group)), ncol = n.col)
    }
  } else {
    if (!identical(group, NA)) {
      scatter <- scatter +
