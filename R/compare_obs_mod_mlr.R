...
compare_obs_mod_mlr <- function(measurements,
  ...
  # check dates
  single_G <- G_matrix %>% dplyr::filter(unit == "normalised")
  ...
  mlr_data <- dplyr::left_join(measurements,
    G_matrix %>%
      dplyr::filter(unit == "normalised") %>%
      dplyr::select(tidyselect::all_of(c("date", mod))),
    by = "date"
  )
  ...
  regr_data <- dplyr::left_join(measurements,
    G_matrix %>%
