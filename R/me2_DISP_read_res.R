...
me2_DISP_read_res <- function(DISPres_file,
  ...
  matrix <- matrix %>%
    ...
    tidyr::pivot_wider(
      id_cols = dplyr::all_of(c(id_variables[1:4], "factor")),
      names_from = "run_type",
      values_from = "value"
    ) %>%
  ...
  if (!tidy_output) {
    data <- matrix %>%
      tidyr::pivot_wider(
        id_cols = dplyr::all_of(id_variables),
        names_from = "factor",
        values_from = "value"
      ) %>%
      arrange(
...
me2_DISP_read_minmax <- function(DISPres_file,
  ...
  output <- me2_DISP_read_res(
    DISPres_file = DISPres_file,
